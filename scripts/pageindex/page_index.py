...
def toc_detector_single_page(content, model=None):
    prompt = f"""
    Your job is to detect if this page contains a TABLE OF CONTENTS (TOC).

    A real table of contents has ALL of these characteristics:
    - It is a list of section/chapter TITLES with corresponding PAGE NUMBERS
    - Each entry is short (just a title and page number), NOT full paragraphs of body text
    - It typically uses dots/leaders (....) or spacing between titles and page numbers
    - It appears near the beginning of a document

    This is NOT a table of contents:
    - A page with numbered lists, bullet points, or headings that are part of the body text
    - A page with section headers followed by paragraphs of content
    - An index, glossary, figure list, abstract, or summary

    Given text: {content}

    return the following JSON format:
    {{
        "thinking": <why do you think this is or is not a table of contents>
...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    ...
    while i < len(page_list):
        ...
        # Skip pages with minimal text — can't be a meaningful TOC
        if len(page_text.strip()) < 100:
            i += 1
            continue
        detected_result = toc_detector_single_page(page_text, model=opt.model)
...
def generate_toc_init(part, model=None):
    ...
    if finish_reason == 'finished':
         ...
         if not result:
             logging.warning(f"generate_toc_init: extract_json returned empty. Response preview: {response[:200]}")
         return result
    else:
        raise Exception(f'finish reason: {finish_reason}')
...
def process_none_page_numbers(toc_items, page_list, start_index=1, model=None):
    for i, item in enumerate(toc_items):
        if "physical_index" not in item:
            ...
            if isinstance(result, list) and len(result) > 0 and isinstance(result[0], dict) and isinstance(result[0].get('physical_index'), str) and result[0]['physical_index'].startswith('<physical_index'):
